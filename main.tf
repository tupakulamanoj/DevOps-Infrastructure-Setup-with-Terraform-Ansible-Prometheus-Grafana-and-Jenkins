...
resource "aws_instance" "ansible_server" {
  ...
  # Ensure Ansible instance is created after the slave instances
...
scrape_configs:
  - job_name: 'node_exporter'
    static_configs:
      - targets: ['localhost:9090','${aws_instance.slave1.private_ip}:9100', '${aws_instance.slave2.private_ip}:9100']
    EOF
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # Run the playbook to deploy the Node Exporter
      ...
      "sudo nohup prometheus --config.file=/etc/prometheus/prometheus.yml > /home/ec2-user/prometheus.log 2>&1 &",
      ...
      "echo 'Prometheus started, monitoring nodes: ${aws_instance.slave1.private_ip}:9100 and ${aws_instance.slave2.private_ip}:9100'"
    ]
...
output "slave2_public_dns" {
  value = aws_instance.slave2.public_dns
}
